...
import { SquaredleSolver } from './algorithm';
import { Trie } from '../dictionary/trie';

// Sample dictionary for worker (same as in loader.ts)
// In production, this would be loaded from a file
const WORKER_DICTIONARY = `
...
self.addEventListener('message', (event) => {
  ...
  try {
    ...
    const words = WORKER_DICTIONARY.filter(word => word.length >= 4);
